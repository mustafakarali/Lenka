...
class CKFinder_Connector_Utils_FileSystem
{
    ...
    public static function checkFolderName($folderName)
    {
        ...
        return self::checkFileName($folderName);
    }
    ...
    public static function checkFolderPath($path)
    {
        ...
        foreach ($path as $dir) {
            if (!empty($dir) && !self::checkFolderName($dir)) {
                return false;
            }
        }
     ...
     *
     * @param string $fileName
     * @param bool   $shortExtensionMode If set to false, extension is everything after a first dot
     *
     * @return string
     */
  ...
  'Û' => 'U', 'Þ' => 'Th', 'Ð' => 'Dh', 'Æ' => 'Ae', 'Ĕ' => 'E',
    ...
    public static function secureFileName($fileName)
    {
        ...
        if ($_config->forceAscii()) {
            $fileName = self::convertToAscii($fileName);
        }
    ...
    {
        $serverPath = self::combinePaths($_resourceType->getDirectory(), $clientPath);
        ...
        while (false !== ($filename = readdir($fh))) {
            if ($filename == '.' || $filename == '..') {
                continue;
            } elseif (is_dir($serverPath.$filename)) {
    ...
    public static function autoRename($filePath, $fileName)
    {
        ...
        $iCounter = 0;
        while (true) {
            $sFilePath = self::combinePaths($filePath, $fileName);
            if (file_exists($sFilePath)) {
                $iCounter++;
                $fileName = self::getFileNameWithoutExtension($sFileNameOrginal, false).'('.$iCounter.')'.'.'.self::getExtension($sFileNameOrginal, false);
            } else {
                break;
            }
        }
    ...
    public static function sendFile($filePath)
    {
        ...
        if ($config->getXSendfile()) {
            self::sendWithXSendfile($filePath);
        } else {
            self::readfileChunked($filePath);
        }
    }
