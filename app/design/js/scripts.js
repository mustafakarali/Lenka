	...
	(function(){
		if($('.product_brands').length){
		 	var pb = $(".product_brands").owlCarousel({
		 		itemsCustom : $('.product_brands').hasClass('with_sidebar') ? [[1199,5],[992,4],[768,3],[480,3],[300,2]] : [[1199,6],[992,5],[768,4],[480,3],[300,2]],
		 		autoPlay : true,
		 		stopOnHover : true,
		 		slideSpeed : 600,
		 		addClassActive : true
		...
		if($('#price').length){
			slider = $('#price').slider({ 
				...
				slide : function(event ,ui){
					$(this).next().find('.first_limit').val(ui.values[0] +' TL');
					$(this).next().find('.last_limit').val(ui.values[1] +' TL');
					
					var minx = $('#price').slider("values",0);
					var maxx = $('#price').slider("values",1);
					
					$('figure.product').each(function() {
						var price = $(this).data('price');
						
						console.log(minx+'<='+price+'<='+maxx);
						
						if (price >= minx && price <= maxx)
							$(this).parent().css('left','').css('position','').css('top','').css('transform','').css('float','left').fadeIn();
						else
							$(this).parent().css('left','').css('position','').css('top','').css('transform','').css('float','left').fadeOut();
					});
				}
			});
