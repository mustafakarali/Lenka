...
class Blog
{
	...
	function __construct()
	{
		...
		if (is_numeric($this->content))
			$this->content = $this->content_id_to_sef($content);
	...
	function content()
	{
		$content = select('contents')
					...
					->where('contents.content_sef = "'.$this->content.'" OR contents.content_id = "'.$this->content.'" ')
					->limit(1)->result();
	...
	function contents()
	{
		...
		if (!empty($this->category))
			$query .= " AND categories.category_sef = '$this->category' OR categories.category_id = '$this->category'";
	...
	function category()
	{
		return $this->format_category(select(self::$table_category)->where('category_sef = "'.$this->category.'" OR category_id = "'.$this->category.'"')->limit(1)->result());
	}
	...
	{
		return select(self::$table_category)
				->left('langs ON langs.lang_id = categories.lang_id')
				->where('parent_id = "'.$category_id.'"')->results();
	}
	...
	{
		return select(self::$table_category)->where('lang_id = "'.$this->lang_id.'"')->results();		
	}
	...
	protected function format_category($result)
	{
		...
		if (!empty($result['category_img']))
			$result['category_img'] = Routes::$image.$result['category_img'];
	...
	function similars()
	{
		...
		$query = $pdo->query("SELECT * FROM contents_similars
							  LEFT JOIN contents ON contents.content_id = contents_similars.similar_id
							  WHERE contents_similars.content_id = '$this->content'");
	...
	function not_similars()
	{
		...
		$other_contents = select('contents')->where('content_id <> "'.$this->content.'" ')->order('content_title ASC')->results();
		// Similar contents
